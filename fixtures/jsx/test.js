...
import { navigationStateChangeHandler } from './utils/routesReport';
...
export default function createNavigationContainer(Component) {
  class NavigationContainer extends React.Component {
    ...
    _onNavigationStateChange(prevNav, nav, action) {
      ...
      navigationStateChangeHandler(prevNav, nav);
    }
    ...
    dispatch = action => {
      ...
      // for MRN 侧滑手势, https://123.sankuai.com/km/page/58963256
      if (Platform.OS === 'ios') {
        NativeModules.MRNContainerControl.enabledPopGestureRecognizer(
          !navState.index
        );
      }
      ...
      dispatchActionEvents();
      // for MRN 关闭容器
      if (action.needCloseContainer) {
        NativeModules.MRNPageRouter.close();
      }
      return false;
    };
